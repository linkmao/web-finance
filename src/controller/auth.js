...
const signUp = async (req, res) => {
  ...
  const newUser = new User({ email, userName, pass: await User.encryptPass(pass), name, lastName,control:[] })
  await newUser.save()
  res.redirect("/") // Despues de crear a cuenta de usuario, se lleva de nuevo a la ruta inicial
  }


const signUpPostman = async (req, res) => {
  const { email, userName, pass, name, lastName } = req.body
  const newUser = new User({ email, userName, pass: await User.encryptPass(pass), name, lastName,control:[] })
  await newUser.save()
...
module.exports = { signUp, signInToken, signUpPostman }
